...
it explains which meteorological variables contribute to fire conditions.
...
their ignition, spread and intensity. Bushfires start if high temperatures,
low levels of relative humidity and precipitation have dried vegetation
to a flammable point. Winds also contribute in the ignition of bushfires
...
danger focusing on the effect of weather on vegetation. Therefore,
this index targets ignition. On the other hand, the Haines Index basis
...
vegetation and topography. This investigation uses the FFDI to explore
...
The computation of the index requires using maximum temperature values.
This variable usually peaks in the afternoon being 3 pm a standard
...
This factor represents the effect of precipitation on fuel loads drying.
...
1 to 10. These values depend on the number of days of the last ``significant''
...
the computation fo the drought factor.
...
\paragraph{Keet-Byram Drought Index (KBDI):}
...
for a maximum value of 100. Over the last years, weather conditions
in catastrophic bushfire events have yileded values above this limit
...
days usually requires the adoption of thresholds or percentiles \citep{Perkins2012}.
Using an arbitrary number of consecutive days that a fixed temperature
value\textemdash also arbitrary\textemdash is exceeded is common \citep{Nairn2013,Perkins2012}.
...
differentiate\textemdash these two types of events. 

In Australia, heatwaves occur because of three mechanisms \citep{McBride2009}.
...
Secondly, the subsidence of air that brings atmospheric stability.
This process produces clear skies that increase solar radiation. Finally,
the increased insolation generates surface heat fluxes. These fluxes
heat the air in the mixing layer. Fire weather comprises these processes
...
climate variability. These drivers are known as climate modes of variability.
For example, El Ni\~no-Southern Oscillation (ENSO), Indian Ocean Dipole
(IOD), and Southern Annular Mode (SAM) are important climate modes
of variability in the Southern Hemisphere \citep{Bjerknes1966,Bjerknes1969,Aceituno1988,Saji1999,ThompsonandSolomon2002}.
...
importance driving precipitation in Victoria, the influence of SAM
...
since these two phases\textemdash warm and cool\textemdash alternate
in periods of approximately four years \citep{NOAA}. 
...
west travel with a westely direction in the upper troposphere. Along
...
parttern is known as ``Walker'' circulation, name coined by \citet{Bjerknes1969}
in honour to Sir Gilbert Thomas Walker. 

A weekening of the easterlies reduces the upwelling in the eastern
part of the tropical Pacific Ocean. This produces an increase in the
sea surface temperature, which transfers heat to the atmosphere weakinning
...
circulation pattern can have effects in places distant where it started
(e.g. floods and droughts). This type of response is referred as ``teleconnection''. 
...
variations in surface pressures along the tropcial Pacific. This is
the main manifestation of a larger phenomena called the ``Southern
...
The index reflects the changes in air-mass fluxes in the tropical
...
Indian Ocean \citep{Saji1999}. This simple index demonstrated to
be useful in representing the variance of SST in this region. 


\subsubsection{Southern Annular Mode}

The Southern Annular Mode (SAM) is the main source of inter-annual
climate variability in the Southern Hemisphere \citep{ThompsonandSolomon2002}.
This refers to the zonally symetric circulation around Antartica.
The SAM produces an alternating pattern of strong and weak westerly
winds in this region. These winds are cold and wet and surround Antartica
as a belt. The SAM drives the expansion and contraction of the circulation. 
...
drives the SAM. Anomalously high pressures in midlatitudes and low
pressures close to the poles strenghten the circumpolar vortex. The
strengthenning process shifts the circulation close to the pole. This
...
Orthogonal Function analysis using 700 hPa geopotential height anomalies. 
...
This section describes the approaches used in seasonal weather forecasting.
...
predictans are temperature and precipitation \citep{Doblas-Reyes2013,Pepler2015,Troccoli2005}.
...
Dynamical models have in general better forecasting accuracy than
...
approaches \citep{Doblas-Reyes2013}. 


\subsubsection{Fire weather forecasting}

Fire weather forecasting use the predictions of variables such as
...
the \textit{Eucalyptus}. They also share ENSO as a strong climate
...
On the other hand, most rural areas of Victoria comprise large forested
areas. Many of them belong to national parks. For example, Grampians
...
81\textsuperscript{o} 04\textquoteright{} W \citep{InstitutoGeograficoMilitarIGM2013a}.
The Galapagos Islands are also part of the country. These islands
are approximately 1000 Km from the continent. The national territory
...
reasons \citep{Anchaluisa2013}. These species are prone to bushfires
...
includes the description of their main climate drivers, regionalization
...
Their negative phases inverse the effect. 
...
are cold and wet. Victoria's climate is also influenced by its topography
...
of 28 \textsuperscript{o}C and maxima of 43 \textsuperscript{o}C.
...
bushfire events occurred in the last 100 years. More general information
...
This region also has an extended fire season. Victoria's fire regime
...
occur in November, March and April.
...
For example, half of the economic loss in Australia occurred in this
...
catastrophes were \textquotedblleft Black Friday\textquotedblright ,
\textquotedblleft Ash Wednesday\textquotedblright , and \textquotedblleft Black
Saturday\textquotedblright{} \citep{DEPIa}. To better understand
...
National Park \citep{DEPIa}. A 10-month drought preceded these fires
...
consecutive days with temperature above 43 \textsuperscript{o}C.
Melbourne\textemdash capital of Victoria\textemdash{} reached this
temperature \citep{VBRC_2009_final_report}. Other areas in Victoria
experienced record-breaking temperatures reaching 46 \textsuperscript{o}C
...
This fire season costed the MDQ more than 50 million dollars \citep{SecretariadeAmbiente2013}.
...
Unfortunately, bushfires in Ecuador are anthropogenic in nature \citep{MinisteriodelAmbiente2013}.
...
bushfires occurrences \citep{Estacio2012}. Rural people also burn
...
bushfire occurrences. The main fire weather variables are temperature,
...
the \textit{Eucalyptus} as the dominant species in bushfire-prone
